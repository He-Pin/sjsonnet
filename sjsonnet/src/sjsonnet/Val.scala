...
sealed abstract class Val extends Eval {
  ...
  /** Runtime type tag for O(1) dispatch in Materializer. */
...
object Val {
  ...
  /** Pre-allocated pool of Val.Num for small non-negative integers 0–255.
    * Used by Evaluator arithmetic fast paths to avoid per-operation allocation.
    * Position is synthetic — acceptable for intermediate runtime results. */
  ...
  /** Returns a cached Val.Num for small non-negative integers (0–255),
    * or a fresh instance otherwise. Use in evaluator arithmetic paths
    * where the pos is not critical for error reporting. */
  ...
  abstract class Builtin1(fn: String, pn1: String, def1: Expr = null)
    ...
    override def apply1(argVal: Eval, outerPos: Position)(implicit
      ...
      if (params.names.length != 1) apply(Array(argVal), null, outerPos)
        ...
        if (tailstrictMode == TailstrictModeEnabled) argVal.value
        evalRhs(argVal.value, ev, outerPos)
      }
  }
