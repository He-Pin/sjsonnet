...
class BaseCharRenderer[T <: upickle.core.CharOps.Output](
  ...
  /** Write a long integer directly into elemBuilder without intermediate String allocation.
    * Uses digit-pair table for fast two-digits-at-a-time conversion. */
...
object BaseCharRenderer {
  // Digit-pair lookup tables for fast two-digit-at-a-time integer rendering.
  // DIGIT_TENS(i) gives the tens digit for value i (0..99).
  // DIGIT_ONES(i) gives the ones digit for value i (0..99).
