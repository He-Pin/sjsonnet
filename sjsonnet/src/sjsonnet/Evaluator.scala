...
class Evaluator(
   ...
   *
   * All body results are wrapped in lazy thunks via visitAsLazy, matching the lazy semantics of C++
   * jsonnet and go-jsonnet. Source elements are only evaluated when the corresponding result
   * element is accessed (e.g., `[x*x for x in [1, 2, error "third"]][0]` succeeds with `1`).
   */
    ...
    case (spec @ ForSpec(_, name, expr)) :: rest =>
      visitExpr(expr)(scope) match {
        case a: Val.Arr =>
          ...
          val len = a.length
          if (rest.isEmpty) {
            // Innermost loop: always produce lazy elements to match go-jsonnet/C++ jsonnet
            // semantics. Source elements are only evaluated when the corresponding result
            // element is accessed.
            var j = 0
            while (j < len) {
