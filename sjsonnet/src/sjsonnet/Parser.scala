        ...
        if (preLocals.length > 0 && exprs.length == preLocals.length) {
          ...
          exprs(preLocals.length) match {
            case Expr.Member.Field(
              ...
              val postLocals = {
                var postEnd = preEnd + 1
                while (postEnd < exprs.length && exprs(postEnd).isInstanceOf[Expr.Bind]) postEnd += 1
                val arr = new Array[Expr.Bind](postEnd - preEnd - 1)
                var j = preEnd + 1; while (j < postEnd) { arr(j - preEnd - 1) = exprs(j).asInstanceOf[Expr.Bind]; j += 1 }
                arr
              }
